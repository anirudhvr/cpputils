...
void
patricia_process (patricia_tree_t *patricia, void_fn_t2 func)
{
...
size_t
patricia_walk_inorder(patricia_node_t *node, void_fn_t2 func)
{
    size_t n = 0;
    assert(func);
...
void deref_data (void *data) {
    assert (data != (void*) 0);
    free(data);
}

patricia_node_t * make_and_lookup2 (patricia_tree_t *tree, char *string)
{
    prefix_t *prefix;
    patricia_node_t *node;
...
patricia_node_t * try_search_best2 (patricia_tree_t *tree, char *string)
{
    prefix_t *prefix;
    patricia_node_t *node;
    ...
    /*
       ...
       printf ("try_search_best: %s/%d (%s) found\n", 
...
patricia_node_t * try_search_exact2 (patricia_tree_t *tree, char
        *string)
{
    prefix_t *prefix;
    patricia_node_t *node;
...
int lookup_then_remove2 (patricia_tree_t *tree, char *string)
{
    patricia_node_t *node;

    if ( (node = try_search_exact2 (tree, string)) ) {
        if (node->data != (void*) 0) 
            free(node->data);
        patricia_remove (tree, node);
        return 1;
